...
- `characters.ts` — 도형 조립. 비율이 핵심이라 머리가 전체 높이의 6할을 차지하고 몸은 그 아래 작게 붙는다.
  얼굴은 머리 아래쪽에 모이고, 눈은 점이 아니라 짙은 테두리 안에 위아래로 흰 조각이 있는 고리다.
  눈썹은 캐릭터마다 다르다. 치이카와는 짧고 낮게, 하치와레는 점처럼, 우사기는 이마를 가로지르는 긴 활이다.
  볼터치는 입체로 만들면 볼에 박혀 보여서, 빛을 받지 않는 납작한 원판을 표면에 붙인다.
